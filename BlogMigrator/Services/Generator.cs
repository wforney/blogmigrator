namespace BlogMigrator
{
    ...
    internal class Generator
    {
        public string GetCategoryById(BlogML.blogType BlogData, int CategoryId)
        {
            ...
            for (var i = 0; i <= BlogData.categories.Length - 1; i++)
            {
        ...
        public void WriteWXRDocument(
            BlogML.blogType BlogData,
            List<int> PostIds,
            string BaseUrl, 
            string FilePath)
        {
            ...
            var writer = new XmlTextWriter(FilePath, new UTF8Encoding(false));
            ...
            for (var i = 0; i <= BlogData.posts.Length - 1; i++)
            {
                ...
                if (PostIds.Contains(Convert.ToInt32(currPost.id)))
                {
                    ...
                    if (currPost.categories != null)
                    {
                        for (var j = 0; j <= currPost.categories.Length - 1; j++)
                        {
                            ...
                            categoryName = this.GetCategoryById(BlogData, Convert.ToInt32(currCatRef.@ref));
                    ...
                    {
                        for (var k = 0; k <= currPost.comments.Length - 1; k++)
                        {
                                     ...
                                     string BaseUrl, string FilePath)
        {
            ...
            // writer.WriteStartElement("item"); writer.WriteElementString("title", string.Join(" ",
            // currPost.title.Text)); writer.WriteElementString("link", BaseUrl + currPost.posturl);
            // writer.WriteElementString("pubDate", currPost.datecreated.ToString("ddd, dd MMM yyyy
            // HH:mm:ss +0000")); writer.WriteStartElement("dc:creator");
            // writer.WriteCData(String.Join(" ", BlogData.authors.author.title.Text));
            // writer.WriteEndElement(); // dc:creator

            // // Post Tags/Categories (currently only categories are implemented with BlogML if
            // (currPost.categories != null) { for (int j = 0; j <= currPost.categories.Length - 1;
            // j++) { currCatRef = currPost.categories[j]; categoryName = GetCategoryById(BlogData,
            // Convert.ToInt32(currCatRef.@ref)); writer.WriteStartElement("category");
            // writer.WriteCData(categoryName); writer.WriteEndElement(); // category
            // writer.WriteStartElement("category"); writer.WriteAttributeString("domain",
            // "category"); writer.WriteAttributeString("nicename", categoryName.ToLower().Replace('
            // ', '-')); writer.WriteCData(categoryName); writer.WriteEndElement(); // category
            // domain=category } }

            // writer.WriteStartElement("guid"); writer.WriteAttributeString("isPermaLink",
            // "false"); writer.WriteString(" "); writer.WriteEndElement(); // guid
            // writer.WriteElementString("description", ".");
            // writer.WriteStartElement("content:encoded");
            // writer.WriteCData(currPost.content.Value); writer.WriteEndElement(); //
            // content:encoded writer.WriteElementString("wp:post_id", currPost.id);
            // writer.WriteElementString("wp:post_date", currPost.datecreated.ToString("yyyy-MM-dd
            // HH:mm:ss")); writer.WriteElementString("wp:post_date_gmt",
            // currPost.datecreated.ToString("yyyy-MM-dd HH:mm:ss"));
            // writer.WriteElementString("wp:comment_status", "open");
            // writer.WriteElementString("wp:ping_status", "open");
            // writer.WriteElementString("wp:post_name", string.Join(" ",
            // currPost.title.Text).ToLower().Replace(' ', '-'));
            // writer.WriteElementString("wp:status", "publish");
            // writer.WriteElementString("wp:post_parent", "0");
            // writer.WriteElementString("wp:menu_order", "0");
            // writer.WriteElementString("wp:post_type", "post");
            // writer.WriteStartElement("wp:post_password"); writer.WriteString(" ");
            // writer.WriteEndElement(); // wp:post_password

            // if (currPost.comments != null) { for (int k = 0; k <= currPost.comments.Length - 1;
            // k++) { currComment = currPost.comments[k]; writer.WriteStartElement("wp:comment");
            // writer.WriteElementString("wp:comment_date",
            // currComment.datecreated.ToString("yyyy-MM-dd HH:mm:ss"));
            // writer.WriteElementString("wp:comment_date_gmt",
            // currComment.datecreated.ToString("yyyy-MM-dd HH:mm:ss"));
            // writer.WriteStartElement("wp:comment_author"); if
            // ((!String.IsNullOrEmpty(currComment.useremail)) || (currComment.useremail !=
            // "http://")) { writer.WriteCData(currComment.username); } else {
            // writer.WriteCData("Nobody"); } writer.WriteEndElement(); // wp:comment_author
            // writer.WriteElementString("wp:comment_author_email", currComment.useremail);
            // writer.WriteElementString("wp:comment_author_url", currComment.userurl);
            // writer.WriteElementString("wp:comment_type", " ");
            // writer.WriteStartElement("wp:comment_content");
            // writer.WriteCData(currComment.content.Value); writer.WriteEndElement(); // wp:comment_content

            // if (currComment.approved) { writer.WriteElementString("wp:comment_approved", null,
            // "1"); } else { writer.WriteElementString("wp:comment_approved", null, "0"); }
